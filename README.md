...
The server also requires the OIDC issuer/client/callback settings plus `APP_BASE_URL` and `SESSION_SECRET`. Phase 1A does not expose login or callback routes yet; these values are validated now so later authentication cannot start with partial configuration. The PostgreSQL-backed session cookie is host-only, `HttpOnly`, `SameSite=Lax`, scoped to `/api`, and has a fixed configurable lifetime (`SESSION_MAX_AGE_MS`, eight hours by default).
...
For production, use `APP_BASE_URL=https://skumanager.ambergalbin.space`, the registered HTTPS callback, `SESSION_COOKIE_SECURE=true`, and the exact trusted proxy-hop count (`TRUST_PROXY=1` for the checked-in nginx-to-server path). The outer TLS proxy must replace untrusted forwarding headers and pass the original HTTPS protocol to nginx. Never expose the server container directly when proxy trust is enabled.
