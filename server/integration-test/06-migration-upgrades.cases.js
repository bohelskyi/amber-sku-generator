  ...
  try {
    const migrationFiles = (await fs.readdir(path.resolve(serverRoot, 'migrations')))
      .filter((fileName) => fileName.endsWith('.sql')
        ...
        && !fileName.startsWith('031_') && !fileName.startsWith('032_') && !fileName.startsWith('035_') && !fileName.startsWith('036_'));
...
test('migration 032 marks generated legacy exposure without confirming its captured revision', async () => {
  ...
  try {
    const migrationFiles = (await fs.readdir(path.resolve(serverRoot, 'migrations')))
      .filter((fileName) => fileName.endsWith('.sql') && !fileName.startsWith('032_') && !fileName.startsWith('035_') && !fileName.startsWith('036_'));
    await Promise.all(migrationFiles.map((fileName) => fs.copyFile(
  ...
  try {
    const migrationFiles = (await fs.readdir(path.resolve(serverRoot, 'migrations')))
      .filter((fileName) => fileName.endsWith('.sql')
        ...
        && !fileName.startsWith('032_') && !fileName.startsWith('035_') && !fileName.startsWith('036_'));
    await Promise.all(migrationFiles.map((fileName) => fs.copyFile(
      path.resolve(serverRoot, 'migrations', fileName), path.resolve(oldDirectory, fileName)
    )));
    await runNodeInDatabase(databaseUrl, `
...
test('legacy zero prices upgrade without repricing products or blocking edits', async () => {
  ...
  try {
    const migrationFiles = (await fs.readdir(path.resolve(serverRoot, 'migrations')))
      .filter((fileName) => fileName.endsWith('.sql') && !/^(014|015|016|029|030|031|032|035|036)_/.test(fileName));
    await Promise.all(migrationFiles.map((fileName) => fs.copyFile(
      path.resolve(serverRoot, 'migrations', fileName),
