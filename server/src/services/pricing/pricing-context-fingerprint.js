...
function getPricingContextSnapshot(context, { legacyDefaultRounding = false } = {}) {
  ...
  return {
    ...
    ...(!legacyDefaultRounding ? {
      marketingRoundingEnabled: Number(context.category?.marketing_rounding_enabled ?? 1),
    } : {}),
    scenarios,
    weightBands,
    matrix,
...
function getPricingContextFingerprint(context) {
  return hashPayload(getPricingContextSnapshot(context));
}
