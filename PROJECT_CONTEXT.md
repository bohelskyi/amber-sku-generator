...
| `021` | Adds distinct direct-recount and export-view capabilities, grants direct recount to Administrator and Storekeeper, grants export viewing to all built-in roles, and corrects the Storekeeper mapping to include product archiving without changing migration 020. |
| `022` | Removes correction claim/release and refresh/complete permissions from Manager while preserving Manager view/create/reject access and the existing Administrator/Storekeeper processing mappings. |
...
- migrations 016–022 legacy-zero, correction-claim, PostgreSQL session-store, local-user/RBAC, and permission-mapping upgrade behavior;
