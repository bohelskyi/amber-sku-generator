...
Authoritative product preview/save/decode, catalog schema versioning, pricing, recount/corrections, in-place price changes, scenario/global repricing, immutable product/price export snapshots, and the Administrator-only audit viewer are implemented. Recount requires a parameter/configuration change and creates correction lineage; price changes preserve product ID, SKU, answers, and schema and support direct permission-driven apply or the existing correction-request lifecycle. Automatic, Manual UAH with optional rounding, and USD/gram modes share authoritative preview/completion semantics. Successful in-place changes advance an exposure-aware coalescing revision for the dedicated `sku,price` stream; normal full-product exports remain separate and the monotonic product cursor never rewinds. Pending price requests mutate neither products nor export state. Product history includes grouped request/price audit events plus a timeline and a `configurationEvolution` projection that never fabricates configuration changes for price requests. See the [recount and corrections guide](docs/RECOUNT_CORRECTIONS.md) and [exports guide](docs/EXPORTS.md).
