...
Authoritative product preview/save/decode, catalog schema versioning, pricing, recount/corrections, in-place price changes, scenario/global repricing, export snapshots, and the Administrator-only audit viewer are implemented. Recount still requires a parameter/configuration change and creates correction lineage; when parameters are unchanged, an authorized separate command can recalculate the existing product automatically or change its Manual-UAH or USD/gram pricing in place without changing its ID, SKU, answers, or schema. Automatic mode clears manual/custom-USD override state and uses the current authoritative pricing configuration, Manual UAH can optionally use authoritative marketing rounding, and USD/gram behavior is unchanged. Successful in-place changes also advance a coalescing product revision so the same SKU can be exported again without rewinding the monotonic product-ID cursor or altering older snapshots. Correction requests can persist a permitted creator's system-automatic, protected USD-per-gram, or exact manual-UAH pricing decision for authoritative completion and later repricing. Product history includes price-only audit events plus a timeline and a `configurationEvolution` projection: the configuration projection reconstructs only recorded configuration states across a correction lineage and reports partial or unavailable evidence rather than inventing missing history. See the [recount and corrections guide](docs/RECOUNT_CORRECTIONS.md).
