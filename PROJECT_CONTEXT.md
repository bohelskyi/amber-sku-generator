...
This document describes the code at commit `5ec5d89` on `feature/postgresql` (2026-09-03), plus any later documentation-only changes. Runtime PostgreSQL configuration is authoritative; a deployed database was not inspected while this document was written.
...
| `server/src/services/repricing.service.js` | Repricing candidates, previews, drafts, manual resolutions, transactional apply and rollback. |
| `server/src/services/correction-request.service.js` | Pending/in-progress/completed/rejected correction-request lifecycle. |
...
- `fixed_uah` — a positive matrix value is the final UAH price and can remain usable if the exchange-rate provider is unavailable;
...
The server returns an authoritative preview and `previewToken`. The token binds normalized answers/calibration, weight, schema version, base SKU, SKU mode, effective price, and effective exchange-rate context. Save requires both the schema-version ID and current token, then rebuilds the preview inside a transaction. A real pricing/schema/answer/weight change after preview causes a stale-preview conflict. Client-supplied calculated SKU or price fields are not trusted.

Save also locks sequence allocation when needed, validates a positive automatic or manual final price, reserves the exact SKU, and writes pricing/answer/schema metadata into `products.details`. Malformed and legacy-shaped payload fields are normalized only where explicitly supported; they do not bypass server validation.
...
Correction requests add a managed pending/in-progress/completed/rejected queue. Only one active request per source is allowed. Signatures detect stale source/proposed state; refresh recalculates; completion invokes the same transactional recount application and records the final payload. Active requests block competing direct correction and repricing. Completion also attempts to synchronize affected repricing drafts.

## Mass repricing

Repricing is scenario-scoped:

- preview loads pricing context and an exchange rate once, finds active matching products, and classifies rows as changed, unchanged, skipped, or error;
- manual-priced products and products lacking a matrix price require an explicit manual resolution rather than being silently overwritten;
- positive manual overrides can resolve only the designated manual/missing-price conditions, not unrelated validation errors;
- the preview token and draft fingerprint bind scenario/configuration, candidates, product state, calculations, and normalized overrides;
- one active draft per scenario stores the preview snapshot, manual overrides, reviewed product IDs, and UI state;
- apply re-previews, rejects stale/conflicting/correction-blocked work, locks affected products in ID order, and writes the batch, product changes, and old/new payloads in one transaction;
- a mid-apply error rolls back the entire batch;
- rollback locks the batch and products, verifies every product is still active and still owned by/exactly matches that batch, then restores every old payload atomically. A later edit prevents unsafe rollback.

Completed apply is idempotent through a unique active preview/application token. Rolled-back batches no longer occupy that active token.
...
Ranges are anchored by existing SKUs but resolved by product ID/creation order, not lexicographic SKU order. Reversed endpoints are normalized. Rows with `exclude_from_export=1` are omitted. CSV contains SKU, rounded UAH price, a derived bracelet/necklace size field, and configured free-text fields.
...
The integration suite compares fresh, pre-checksum legacy, and checkpoint-upgrade topology and tests repeated startup, failed-file rollback, timeout independence, checksum normalization, and migration 016 compatibility. This covers the repository's known upgrade shapes; it is not a guarantee for an arbitrary manually altered database.
...
- one active correction request and one active repricing draft per logical owner are enforced by partial unique indexes;
...
- liveness/readiness;
- calibrated-to-semi-calibrated recount with target-hidden answer removal, legacy hidden zero, and visible-target rejection;
...
- migration 016 legacy-zero upgrade/edit behavior;
...
- manual-price correction, concurrent correction, correction requests, and repricing blocking;
- exchange-cache ordering;
- category-code immutability;
- repricing apply/rollback/manual cycles and forced mid-apply rollback;
...
- In recount only, drop inherited answers hidden by the target configuration; keep target-visible and all ordinary new-product validation strict.
...
- Keep repricing apply/rollback atomic and explicit for manual-priced/missing-price products.
- Keep export CSV immutable after generation, idempotency range-bound, formula-safe, and cursor advancement monotonic.

## Current status

The PostgreSQL architecture and audit fixes through migrations `000`–`016` are present on `feature/postgresql`. The latest code includes generalized target-hidden recount cleanup and cross-platform migration checksum normalization. CI configuration and focused unit/integration regression coverage are present. Security issues #1 and #2 remain deliberately unresolved, and actual production configuration/data must be checked operationally rather than inferred from this checkout.
