...
**Phase 2 status (2026-09-26):** full-product lifecycle state, immutable exact membership, request parity, informational/name revisions and inherited-name review are implemented. Normal selection still uses the existing cursor and exclusion rules. Recount successors remain excluded, including those with ledger route `normal`. **Phase 2 is NOT independently deployable as the full correctness fix.** The release blocker remains. See [the Phase 2 behavior below](#phase-2-name-review-and-in-place-full-revisions--2026-09-26).
...
## Phase 1 full-product lifecycle — 2026-09-26

Migration 039 adds `product_full_export_state` independently of `product_export_revisions`. Its positive current `revision`, monotonic `confirmed_revision` and positive `delivery_version` describe full-product payload and routing state. Routes are `normal`, `replacement`, `hold` and `retired`; hold reasons distinguish historical ambiguity, prior exposure, intentional exclusion and invalid lineage. `exports.reconcile` is delegable and initially Administrator-only, but has no command/UI yet. Existing active products receive conservative historical holds; corrected/archived products receive retired state. No existing exclusion, cursor or snapshot bytes are changed by the migration.

Every new snapshot records immutable `full_product_lifecycle_version=1`. Normal Magento captures store one immutable `export_snapshot_products` row per product, even though Main/EN artifacts contain two CSV rows. The service parses the actual stored internal and artifact CSV through the trusted Phase 0 evidence parser and validates product count, SKU, group and Main/EN membership. Members bind exact SKU, full revision, delivery version, `full_product` capture kind, `live_capture` origin and a SHA-256 hash covering the represented immutable files and counters. Artifacts, members, snapshot, price exposure and audit commit or roll back together. UPDATE/DELETE/TRUNCATE of membership is forbidden.

Internal-only compatibility snapshots also record exact product membership but use `legacy_compatibility` with null full/delivery counters, so they cannot acknowledge a full-product revision. Historical snapshots retain NULL lifecycle provenance and their original confirmation behavior. No historical membership is synthesized, no old snapshot is regenerated, and stored-key retries return their original stored result before requiring current preview/catalog/lifecycle evidence. `recorded_at` is the real membership insertion time, not a fabricated historical capture date.

All new captures, including the legacy mapper/compatibility path, now use REPEATABLE READ. Their order is existing access/session protection → idempotency coordination → template selection when applicable → ascending products → ascending full state → ascending price revisions → new-mode cursor → snapshot/artifacts/members/audit. Legacy and published preview fingerprints bind full lifecycle counters/route as well as their existing inputs. A concurrent changed row can produce 409 `EXPORT_PREVIEW_STALE`; the caller must refresh. A completed idempotent winner is recovered only after rollback with a fresh committed lookup.

Confirmation locks snapshot → ascending full state → ascending price revisions → cursor, without acquiring current product locks after lifecycle locks. It validates lifecycle version and exact membership, then advances only each member's captured full revision with `GREATEST`. Confirming revision 1 after revision 2 exists leaves revision 2 pending; out-of-order/repeated confirmation cannot regress the high-water mark. A predecessor member never acknowledges its successor, and later source retirement does not invalidate confirmation of an already generated file. First confirmer, audit idempotency, original bytes and monotonic legacy cursor advancement remain intact.

Price-only changes and confirmation remain owned by `product_export_revisions` and the dedicated price snapshot flow. Full capture still establishes existing price exposure, and full confirmation applies only existing captured price evidence; it cannot clear later price revisions. Phase 1 introduced the internal CAS primitive; Phase 2 now uses it for informational/name edits as described below.

Lifecycle routes remain foundation state without a new export selector. Holds do not become selection gates here, and a `normal` recount successor remains excluded by the old production selector. **Do not deploy Phase 1 independently.** Final activation must not run mixed old/new writers. Phase 2 request/in-place parity and inherited-name review are described below. Phase 3 data repair, Phase 4 ledger selection/UI and Phase 5 rollout remain unimplemented. See [Phase 1 implementation and verification evidence](RECOUNT_EXPORT_CORRECTNESS_PLAN.md#19-phase-1-actual-results--2026-09-26).

## Phase 2 name review and in-place full revisions — 2026-09-26

This section extends the historical Phase 1 boundary above. Ordinary cursor/exclusion selection, New/manual range behavior, snapshot membership format and confirmation algorithm are unchanged. There is no pending-update/replacement queue, held-products dashboard or release command. The 997 excluded active historical successors and product 4848 have not been repaired or released.

Successful edits of existing approved informational fields and saved manual Magento subjects now increment `product_full_export_state.revision` in the same transaction as the product mutation and audit. The informational allowlist and dynamic catalog guards are unchanged; `SV.weight` remains unsupported. Rejected/no-op edits and previews do not advance revisions. These changes keep ID, SKU, schema, lineage, price and exclusion. They never increment `product_export_revisions`; price-only direct/request changes retain that separate stream and do not increment full revision.

Before first full confirmation, an edit changes revision 1/confirmed 0 to 2/0. After confirmation it changes 1/1 to 2/1. A snapshot generated at N retains immutable membership at N; a later information/name edit creates N+1, and confirming the old file acknowledges at most N. The pending delta remains visible in lifecycle state for future Phase 4 selection. Repeated and out-of-order confirmations preserve the high-water mark. Main/EN rows still produce exactly one member per product. No current payload is regenerated when confirming a historical file.

`magento_name_review_required=true` is now a domain readiness error (`manual_name_review_required`, field `name`) in the legacy Magento mapper and the existing template evaluator, including draft/published/session review through their shared input loaders. Frozen definitions and CSV contracts are unchanged; the flag is not a new configurable template source. A saved pair alone cannot make an unreviewed inherited name ready. The existing name editor shows **Потрібна перевірка успадкованих назв**, loads the authoritative pair and supports editing it or **Підтвердити без змін**. Read/download of stored files remains independent of current readiness.

`POST /api/product-magento-name/preview` with only `productId` reads the current pair and returns `reviewRequired`, `canConfirmUnchanged`, and a `previewToken` only when unchanged confirmation is eligible. Edited-name preview uses the existing `subjectUa`/`subjectEn` contract. Explicit unchanged confirmation sends both subjects, `confirmUnchanged: true` and the token for the pair actually displayed. The client does not silently acquire new proof when that button is pressed. Apply rechecks active-product, exact pair, review and full-lifecycle evidence under product → full-state locks. A stale token returns `STALE_MAGENTO_NAME`; ordinary no-change mutation remains `NO_CHANGE`.
