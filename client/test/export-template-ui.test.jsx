...
const approvedRegistry = () => ({ productFields: ['full_sku', 'weight', 'total_price_uah'], references: { questions: [], schemas: baseline().groups.map((g) => ({ category_code: g.route, questions: Object.values(baseline().sources).filter((s) => s.category === g.route).map((s) => ({ key: s.key, value_ids: [] })) })) } });
const all = ['export_templates.view', 'export_templates.manage', 'export_templates.publish', 'export_templates.activate', 'exports.view'];
const { sourceSupportUpdate } = require('../../server/src/services/export-templates/source-support');
const family = (id = 'family', definition = baseline()) => ({ id, display_name: `Шаблон ${id}`, draft: { revision: '9007199254740993', definitionHash: hashJsonData(definition), definition, sourceSupportUpdate: sourceSupportUpdate(definition) }, versions: [] });
...
const normalOutput = () => {
  const row = screen.queryByRole('region', { name: 'Умова 1' });
  return row ? within(row.querySelector('.et-condition-result')) : screen;
};
const choose = (label, value) => {
  if (label === 'Категорія') { fireEvent.click(screen.getAllByRole('tab')[Number(value)]); return; }
  if (label === 'Мова') {
    const selected = screen.getByRole('button', { pressed: true, name: / \/ (Основний|EN)$/ }).getAttribute('aria-label').split(' / ');
    fireEvent.click(screen.getByRole('button', { name: `${selected[0]} / ${selected[1]} / ${value === '1' ? 'EN' : 'Основний'}` })); return;
  }
  if (label === 'Шаблон') {
...
const click = (name) => {
  ...
  fireEvent.click(scope.queryByRole('button', { name: mapped, exact: true }) || screen.getByRole('link', { name: mapped, exact: true }));
};
beforeEach(() => {
  ...
  api.sources.mockResolvedValue(response({ productFields: ['full_sku', 'weight', 'total_price_uah'], references: { questions: [], schemas: baseline().groups.map((g) => ({ category_code: g.route, questions: Object.values(baseline().sources).filter((s) => s.category === g.route).map((s) => ({ key: s.key, value_ids: [] })) })) } }));
...
it('direct forbidden navigation issues no definition/source requests; delegated view+publish works independently', async () => {
  ...
  const allowed = page(['export_templates.view', 'export_templates.publish']); await screen.findAllByRole('link', { name: /^Відкрити Шаблон/ }); choose('Шаблон', f.id); await screen.findByRole('tablist', { name: 'Категорії файлів' });
...
it('sample selection fences late results; a failed new sample cannot leave an old sample marked current', async () => {
  ...
  click('Переглянути результат'); await screen.findAllByText(/Шаблон ще не готовий до публікації/);
  expect(screen.queryByRole('region', { name: 'Тест чернетки' })).toBeNull();
});
...
it('late save cannot replace a newer loaded family, and late draft preview cannot validate newer edits', async () => {
  ...
  page(); await screen.findAllByRole('link', { name: /^Відкрити Шаблон/ }); choose('Шаблон', 'a'); await screen.findByRole('tablist', { name: 'Категорії файлів' });
...
it('manage without exports.view cannot issue draft previews; permission denial does not retry', async () => {
  ...
  page(['export_templates.view', 'export_templates.manage']); await screen.findAllByRole('link', { name: /^Відкрити Шаблон/ }); choose('Шаблон', f.id); await screen.findByRole('tablist', { name: 'Категорії файлів' });
...
it('dirty router navigation supports Stay, failed Save, Discard and history back', async () => {
  ...
  await act(async () => router.navigate(-1)); await screen.findByRole('tablist', { name: 'Категорії файлів' });
...
it('named creation prepares a technical key once, keeps conflicts editable and prevents duplicate submissions', async () => {
  ...
  expect(screen.queryByRole('tablist', { name: 'Категорії файлів' })).toBeNull();
  ...
  click('Створити й зберегти чернетку'); await screen.findByRole('tablist', { name: 'Категорії файлів' });
  expect(api.publish).not.toHaveBeenCalled(); expect(api.select).not.toHaveBeenCalled();
});
...
it('ordinary KL name workflow adds mapped color, edits only this field, saves and inspects exact server revision without advanced rules', async () => {
  ...
  page(); await screen.findAllByRole('link', { name: /^Відкрити Шаблон/ }); choose('Шаблон', f.id); await screen.findByRole('tablist', { name: 'Категорії файлів' });
  choose('Категорія', '2'); choose('Колонка', 'name');
...
it('focused tokens support rename, explicit removal and dangling-reference validation', () => {
  ...
  function Focused() {
    ...
    return <FieldEditor definition={definition} registry={approvedRegistry()} onChange={(next) => { current = next; setDefinition(next); }} />;
  }
  render(<Focused />); choose('Категорія', '2'); choose('Колонка', 'name'); click('+ Додати характеристику');
  choose('Характеристика', 'KL.color'); choose('Як записувати значення', 'klColor'); click('Вставити характеристику');
  expect(normalOutput().getByLabelText('Текст у файлі').checkValidity()).toBe(true);
  click('Технічні подробиці'); click('Колір {Колір}'); fireEvent.click(screen.getByText('Назва та вилучення характеристики'));
  choose('Назва підстановки', 'shade'); click('Перейменувати й оновити текст');
  click('← Звичайні налаштування');
  expect(normalOutput().getByLabelText('Текст у файлі').value).toContain('{Колір}');
  click('Колір {Колір}');
  click('Вилучити характеристику та її позначки з тексту');
  click('Застосувати до чернетки'); expect(current.groups[2].rows[0].cells.name.then.slots.shade).toBeUndefined(); choose('Колонка', 'name');
  expect(normalOutput().getByLabelText('Текст у файлі').value).not.toContain('{Колір}');
  choose('Текст у файлі', '{unknown}');
  expect(normalOutput().getByLabelText('Текст у файлі').checkValidity()).toBe(false);
...
it('changing principal discards old workspace and ignores a late family response', async () => {
  ...
  expect(screen.queryByRole('tablist', { name: 'Категорії файлів' })).toBeNull(); expect(screen.queryByRole('button', { name: 'Зберегти чернетку' })).toBeNull();
});

it('read-only fields allow inspecting characteristics and branches without edit actions or source requests', async () => {
  const change = vi.fn(); const original = baseline();
  render(<FieldEditor definition={original} onChange={change} readOnly />);
  choose('Категорія', '2'); choose('Колонка', 'name'); click('Матеріал {Матеріал}');
  expect(screen.getByLabelText('Значення у CSV: Значення №1 — назву не підтверджено').matches(':disabled')).toBe(true);
  expect(screen.queryByRole('button', { name: '+ Додати характеристику' })).toBeNull();
  expect(screen.queryByLabelText('Де застосувати зміни')).toBeNull();
  expect(within(screen.getByRole('region', { name: 'Інакше' })).getByLabelText('Текст у файлі').matches(':disabled')).toBe(true);
...
it('focused shared mapping warning names the actual consumers before an explicit shared edit', () => {
  ...
  choose('Категорія', '2'); choose('Колонка', 'name'); click('Технічні подробиці'); choose('Де застосувати зміни', 'shared'); click('Матеріал {Матеріал}');
  ...
  choose('Значення у CSV: Значення №1 — назву не підтверджено', 'Спільний матеріал'); click('Застосувати до чернетки');
...
it('normal saved-draft task searches SKU, selects samples, reads server table and opens source diagnostics without losing edits', async () => {
  ...
  fireEvent.click(screen.getAllByRole('button', { name: /Картини.*Основний.*Розмір/ })[0]);
  await screen.findByRole('heading', { name: 'Розмір картини' });
  expect(screen.getByRole('tab', { name: 'Картини' }).getAttribute('aria-selected')).toBe('true'); expect(screen.getByRole('button', { name: 'AR / rozmir_kartyny / Основний' }).getAttribute('aria-pressed')).toBe('true');
  expect(screen.queryByText('Подробиці джерела')).toBeNull(); expect(screen.getByRole('button', { name: 'Технічні подробиці', exact: true })).toBeTruthy();
  choose('Значення у CSV: Значення №1 — назву не підтверджено', 'Локальна зміна'); click('Застосувати до чернетки'); click('Перевірка');
  fireEvent.click(screen.getAllByRole('button', { name: /Намиста.*Основний.*Додатково/ })[0]);
  ...
  expect(screen.getByLabelText('Значення у CSV: Значення №1 — назву не підтверджено').value).toBe('Локальна зміна');
...
it('empty registry exposes actual code-backed system tables without writes and explicit copy uses the draft API', async () => {
  ...
  const current = upgradeSourceSupport(baseline(), officeEvidence());
  api.system.mockResolvedValue(response({ definition: baseline(), kind: 'system' }));
  api.candidate.mockResolvedValue(response({ definition: current, diagnostics: [] }));
  api.create.mockResolvedValue(response(family('system-copy', current)));
  page(); await screen.findByRole('link', { name: 'Переглянути' });
  click('Переглянути');
  ...
  for (const group of baseline().groups) {
    fireEvent.click(within(screen.getByRole('tablist', { name: 'Категорії файлів' })).getByRole('tab', { name: group.name, exact: true }));
    const table = screen.getByRole('table');
    expect([...table.querySelectorAll('thead code')].map((e) => e.textContent)).toEqual(group.columns);
  }
  click('Створити редаговану копію'); await screen.findByLabelText('Назва шаблону'); choose('Назва шаблону', 'Explicit synthetic copy');
...
it.each(['Створити шаблон', 'Створити редаговану копію'])('SUPPORT current creation via %s needs no compatibility opt-in', async (action) => {
  const { officeEvidence } = require('../../server/test/fixtures/export-source-support');
  const { upgradeSourceSupport } = require('../../server/src/services/export-templates/source-support');
  const next = upgradeSourceSupport(baseline(), officeEvidence());
  api.candidate.mockResolvedValue(response({ definition: next, diagnostics: [] }));
  api.create.mockResolvedValue(response(family('new-current', next)));
  page(); await screen.findByText(/Шаблонів ще немає/); click(action); await screen.findByText('Новий шаблон');
  choose('Назва шаблону', 'Synthetic current policy');
...
it.each(['current', 'unsupported'])('SUPPORT uses authoritative %s status without guessing availability from the definition', async (status) => {
  const f = family('policy-status');
...
it('SUPPORT a no-delta prepare response removes the action without offering application', async () => {
  const f = family('no-delta');
  api.list.mockResolvedValue(response({ templates: [f] })); api.get.mockResolvedValue(response(f));
  ...
  expect(screen.queryByRole('dialog')).toBeNull(); expect(api.applySupport).not.toHaveBeenCalled();
});

it('UX2 registry separates system, draft, latest publication and an older selected candidate using summaries only', async () => {
  const summaries = [
    { id: 'alpha', display_name: 'Каталог прикрас', draft_revision: '7', publication_count: 2, selected_version_id: 'v1', version_summaries: [
      { id: 'v2', versionNumber: '2', sourceDraftRevision: '6' }, { id: 'v1', versionNumber: '1', sourceDraftRevision: '5' },
    ] },
    { id: 'beta', display_name: 'Картини', draft_revision: '1', publication_count: 0, version_summaries: [] },
  ];
  ...
  expect(screen.getAllByText('Опублікована версія · незмінна')).toHaveLength(2);
  expect(api.get).not.toHaveBeenCalled(); expect(api.system).not.toHaveBeenCalled(); expect(api.save).not.toHaveBeenCalled();
});

it('UX2 publish-only capability reviews and publishes the exact saved revision without manage validation', async () => {
  const f = family(); api.list.mockResolvedValue(response({ templates: [f] })); api.get.mockResolvedValue(response(f));
  api.publish.mockResolvedValue(response({ id: 'published', versionNumber: '1', definition: f.draft.definition, definitionHash: f.draft.definitionHash }));
  page(['export_templates.view', 'export_templates.publish']); await screen.findByRole('link', { name: 'Відкрити Шаблон family' }); choose('Шаблон', f.id);
  await screen.findByRole('tablist'); click('Версії'); click('Опублікувати версію');
  const review = screen.getByRole('dialog', { name: 'Перегляд публікації' }); expect(review.textContent).toContain('Шаблон family'); expect(review.textContent).toContain(f.draft.revision);
  expect(api.publish).not.toHaveBeenCalled(); click('Підтвердити публікацію');
  await waitFor(() => expect(api.publish).toHaveBeenCalledWith(f.id, { expectedRevision: f.draft.revision, expectedDefinitionHash: f.draft.definitionHash }));
  expect(api.validate).not.toHaveBeenCalled(); expect(api.sources).not.toHaveBeenCalled(); expect(api.select).not.toHaveBeenCalled();
  await screen.findByText('Опубліковано v1. Вибір для експорту не змінено.');
});

it('UX2 manage-only cannot publish or activate, and unknown draft rules survive open/cancel/no-op server save', async () => {
  const { upgradeColumns } = require('../../server/src/services/export-templates/column-contract');
  const d = upgradeColumns(baseline()); d.groups[0].rows[0].cells.meta_title = { op: 'futureRule', opaque: [null, 0, '0', '', '  '] };
  const f = family('custom', d); api.list.mockResolvedValue(response({ templates: [f] })); api.get.mockResolvedValue(response(f)); api.save.mockResolvedValue(response(f.draft));
  page(['export_templates.view', 'export_templates.manage']); await screen.findByRole('link', { name: 'Відкрити Шаблон custom' }); choose('Шаблон', f.id);
  await screen.findByRole('tablist'); choose('Колонка', 'meta_title'); expect(screen.getByText(/Власне правило збережено/)).toBeTruthy(); click('Скасувати');
  click('Зберегти чернетку'); await waitFor(() => expect(api.save).toHaveBeenCalledTimes(1));
  expect(hashJsonData(api.save.mock.calls[0][1].definition)).toBe(f.draft.definitionHash);
  click('Версії'); expect(screen.queryByRole('button', { name: 'Опублікувати версію' })).toBeNull(); expect(screen.queryByRole('button', { name: 'Скасувати вибір шаблону' })).toBeNull();
  expect(api.publish).not.toHaveBeenCalled(); expect(api.select).not.toHaveBeenCalled();
});

it('UX2 page Save rejects pending inspector input; clean no-op save leaves the same open column editable', async () => {
  const f = family(); api.list.mockResolvedValue(response({ templates: [f] })); api.get.mockResolvedValue(response(f));
...
it('UX2 source issue selects its explicit EN consumer and source without changing the saved definition', async () => {
  const f = family(); api.list.mockResolvedValue(response({ templates: [f] })); api.get.mockResolvedValue(response(f));
  api.validate.mockRejectedValue({ response: { data: { code: 'TEMPLATE_SOURCE_INVALID', details: { diagnostics: [{ code: 'SOURCE_REFERENCE_UNRESOLVED', sourceId: 'BR.raw_type', category: 'BR', key: 'raw_type' }] } } } });
  page(); await screen.findByRole('link', { name: 'Відкрити Шаблон family' }); choose('Шаблон', f.id); await screen.findByRole('tablist');
  click('Перевірити шаблон'); const target = await screen.findByRole('button', { name: 'Браслети → Назва товару · EN · Матеріал' });
  expect(screen.getByText('TEMPLATE_SOURCE_INVALID').closest('details').open).toBe(false);
  fireEvent.click(target);
  expect(screen.getByRole('button', { name: 'BR / name / EN' }).getAttribute('aria-pressed')).toBe('true');
  expect(document.querySelector('.et-intent-editor pre')).toBeNull(); expect(screen.getByRole('button', { name: 'Технічні подробиці', exact: true })).toBeTruthy(); expect(screen.getByRole('tab', { name: 'Браслети' }).getAttribute('aria-selected')).toBe('true');
  click('Скасувати'); api.save.mockResolvedValue(response(f.draft)); click('Зберегти чернетку'); await waitFor(() => expect(api.save).toHaveBeenCalledTimes(1));
  expect(hashJsonData(api.save.mock.calls[0][1].definition)).toBe(f.draft.definitionHash);
});
