...
function SessionDetail({ id, canCreate, canActivate, register, onLost }) {
  ...
  const { beginExportHandoff } = useContext(ExportWorkflowContext) || {};
  ...
  const denied = useCallback((status) => { if (current()) { accessible.current = false; readTicket.current++; reviewTicket.current++; viewMemory.clear(); setSession(null); setValue(null); setPreview(null); setSharing(false); setManualName(null); setDirty(false); dirtyRef.current = false; onLost(status); } }, [current, onLost, viewMemory]);
  const handleError = useCallback((e) => {
    if (!current()) return;
    if ([401,403,404].includes(e.response?.status)) denied(e.response.status);
    else {
  ...
  const check = useCallback(async (automatic = false) => {
    ...
    setReviewStale(true); setRefreshing(automatic);
    try { await run(() => api.preview(id), (data) => { if (ticket !== reviewTicket.current) return; setPreview(data); setReviewStale(false); setProductChanged(false); viewMemory.update({ productChanged: false }); }); }
    finally { if (current()) setRefreshing(false); }
  }, [current, run, id, viewMemory]);
  useEffect(() => {
    if (!session || session.snapshotId || session.executing || dirty || busy || !(viewMemory.read()?.autoRecheck || viewMemory.read()?.returnReview)) return;
    const timer = window.setTimeout(() => {
      const automatic = Boolean(viewMemory.read()?.autoRecheck);
      viewMemory.update({ autoRecheck: false, returnReview: false }); void check(automatic);
    }, 0);
    return () => window.clearTimeout(timer);
  }, [session, dirty, busy, viewMemory, check]);
  ...
  return <section className={preview ? 'space-y-2' : 'space-y-4'}>
    <header className="export-session-header">
      <div className="space-y-2"><div className="flex flex-wrap items-center gap-3"><h2>{session.title}</h2><span className="export-workspace-status" role="status">{status}</span></div>
        ...
        {!preview && <p className="text-sm text-slate-600">{sessionRecipe(session)}</p>}</div>
    ...
    {!frozen && <>
      <details open={(!session.currentAttemptId && !preview) || dirty} className="space-y-3"><summary>Налаштування експорту · {rangeText(value.settings)}</summary><Settings value={value} disabled={busy || !canCreate || session.executing} canActivate={canActivate}
      ...
      <section ref={recovery} aria-label="Відновлення" className={preview && !recovering && !session.executing && !preparationIssue && preparationMatches ? 'space-y-2' : 'export-recovery space-y-3'}>
        {session.executing ? <p>{session.attempt?.state === 'executing' ? 'Сервер створює файли. Стан оновлюється; дочекайтеся результату.' : 'Сервер виконує дію з експортом. Дочекайтеся оновлення стану.'}</p>
          : recovering ? <p>Створення не завершено або відповідь не отримано. Повтор відновлює початкове створення цього експорту, навіть якщо ви вже переглянули новішу перевірку.</p>
            : session.currentAttemptId && !tableAvailable ? <p>Перевірку збережено. Перед першим створенням файлів перевірте товари й перегляньте актуальну таблицю.</p>
              : !preview && <p>Перевірте товари, збережіть перевірку та створіть файли окремою дією.</p>}
    ...
    {!frozen && preview && <div className="card">
      <ExportReview preview={{ ...p, checkedAt: p.checkedAt || session.attempt?.preparedAt }} stale={!reviewCurrent || !tableAvailable} busy={busy || dirty || session.executing}
        productChanged={productChanged} refreshing={refreshing} viewMemory={viewMemory} recoveryFocusBlocked={Boolean(manualName)} onRefresh={() => check()} onEditName={canCreate ? setManualName : undefined}
        canDecode={permissions.includes('products.view') && permissions.includes('products.decode')} onHandoff={(context) => { viewMemory.update({ returnReview: true }); beginExportHandoff?.({ ...context, sessionId: id, returnTo: '/exports/sessions/' + encodeURIComponent(id) }); }} />
      {manualName && <WorkspaceDialog title="Назва товару Magento" onClose={() => setManualName(null)}><ManualMagentoNameEditor product={manualName}
        onClose={() => setManualName(null)} onSaved={() => { setManualName(null); setReviewStale(true); setProductChanged(true); void check(true); }} translationSuggestionAvailable={false} /></WorkspaceDialog>}
    </div>}
...
function SessionWorkspace({ canCreate, canActivate, scope = 'owned', create = false }) {
  ...
  const lost = useCallback((status) => {
    listTicket.current++; cache.current = {}; setItems([]); setNext(null); setListBusy(false);
    setSelected(null); setKnown(''); setKnownId(''); setForm({ dirty: false, busy: false });
    setNotice(status === 404 ? 'Експорт не знайдено або він недоступний для вашого облікового запису.' : 'Доступ до цього експорту втрачено. Дані закрито.');
  }, []);
