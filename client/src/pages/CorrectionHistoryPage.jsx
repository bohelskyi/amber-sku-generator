      ...
      <main className="mx-auto w-full min-w-0 max-w-7xl space-y-5 overflow-hidden px-4 py-4 pb-20 sm:px-6 sm:py-6">
        ...
        <section className="card grid grid-cols-2 overflow-hidden lg:grid-cols-5">
          {[
            ...
            <div
              key={label}
              className={`border-b border-r border-slate-200 px-4 py-3 last:border-r-0 sm:px-5 lg:border-b-0 ${label === 'Сумарна різниця' ? 'col-span-2 lg:col-span-1' : ''}`}
            >
        ...
        <section className="card min-w-0 overflow-hidden">
          <div className="grid gap-3 border-b border-slate-200 p-3 sm:p-4 lg:grid-cols-[minmax(220px,1fr)_180px_160px_160px_auto] lg:items-end">
            ...
            <div className="divide-y divide-slate-200">
              {items.map((item) => (
                <article key={item.id} className="p-4">
                  ...
                  <div className="grid gap-4 xl:grid-cols-[minmax(360px,1.1fr)_minmax(300px,1fr)_minmax(250px,0.7fr)]">
                    <SkuTransition item={item} />

                    <div className="min-w-0">
                      <div className="mb-2 text-xs font-semibold uppercase text-slate-500">Змінені характеристики</div>
                      <div className="history-change-list divide-y divide-slate-200">
                        {item.changes.map((change) => (
                          <div key={change.key} className="change-record-row text-sm">
                            <span className="change-record-label font-medium text-slate-700">{change.questionLabel}</span>
                            <span className="change-record-old text-slate-500">{change.fromLabel}</span>
                            <ArrowRight size={13} className="change-record-arrow text-slate-400" />
                            <strong className="change-record-new font-semibold text-slate-900">{change.toLabel}</strong>
                          </div>
                        ))}
                      </div>
                    </div>

                    <div className="history-price-details min-w-0 border-t border-slate-200 pt-3 xl:border-l xl:border-t-0 xl:pl-4 xl:pt-0">
